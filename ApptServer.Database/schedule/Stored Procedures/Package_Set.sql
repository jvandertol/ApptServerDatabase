...
-- =============================================
CREATE PROCEDURE [schedule].[Package_Set]
...
-- security.CoExternalCoAssoc table exists because the ExternalCompanyId column does not exist in the Company table and it should.  The query would still exist but it would go against the company table
	...
	if @PackageId is null begin
		...
		( PackageName,SinglePriceFlag,CompanyId,PackageXml,EstDurationMins,ShowPriceOnline,Price,IsDeleted)
		values (
			 @PackageName,@SinglePriceFlag,@CompanyId,@PackageXml,@EstDurationMins,@ShowPriceOnline,@Price,0
		)
